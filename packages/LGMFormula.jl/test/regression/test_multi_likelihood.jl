...
@testset "PR-4 multi-likelihood tuple-LHS roundtrip" begin
    ...
    @testset "bivariate Gaussian + Intercept" begin
        model_macro = @lgm (y_real, y_real2) ~ 1 data=df family=(GaussianLikelihood(), GaussianLikelihood())
        A = sparse(ones(n, 1))
    ...
    @testset "Gaussian + Poisson + Intercept + IID" begin
        model_macro = @lgm (y_real, y_count) ~ 1 + f(idx, IID(5)) data=df family=(GaussianLikelihood(), PoissonLikelihood())
    ...
    @testset "three-likelihood: Gaussian + Poisson + Gaussian" begin
        model_macro = @lgm (y_real, y_count, y_real2) ~ 1 + x data=df family=(GaussianLikelihood(), PoissonLikelihood(), GaussianLikelihood())
    ...
    @testset "no intercept + 1 covariate + IID" begin
        model_macro = @lgm (y_real, y_count) ~ 0 + x + f(idx, IID(5)) data=df family=(GaussianLikelihood(), PoissonLikelihood())
        X = sparse(reshape(Float64.(df.x), n, 1))
        A_iid = sparse(1:n, df.idx, 1.0, n, 5)
        A = hcat(X, A_iid)
        proj = LinearProjector(A)
...
@testset "PR-4 lgmformula function form (tuple-LHS)" begin
    ...
    @testset "function form matches macro form" begin
        model_macro = @lgm (y1, y2) ~ 1 + x + f(idx, IID(4)) data=df family=(GaussianLikelihood(), PoissonLikelihood())
...
@testset "PR-4 @macroexpand structure (tuple-LHS)" begin
    ...
    @testset "expansion uses _build_multi_likelihood_mapping" begin
        ex = @macroexpand @lgm (y1, y2) ~ 1 data=df_dummy family=(GaussianLikelihood(), PoissonLikelihood())
    ...
    @testset "components tuple is shared (not duplicated per likelihood)" begin
        ex = @macroexpand @lgm (y1, y2) ~ 1 + x + f(idx, IID(4)) data=df_dummy family=(GaussianLikelihood(), PoissonLikelihood())
        call = _find_lgm_call(ex)
...
@testset "PR-4 error messages refer to user concepts (tuple-LHS)" begin
    ...
    @testset "unknown LHS column in tuple" begin
        @test_throws ArgumentError begin
            @lgm (y1, missing_y) ~ 1 + x data=df family=(GaussianLikelihood(), GaussianLikelihood())
        end
        try
            @lgm (y1, missing_y) ~ 1 + x data=df family=(GaussianLikelihood(), GaussianLikelihood())
        catch e
    ...
    @testset "tuple-LHS entry is non-symbol" begin
        ex = try
            @eval @lgm ($("y1"), y2) ~ 1 data=$df family=(GaussianLikelihood(), GaussianLikelihood())
            nothing
        catch e
            e
        end
