...
| `vb_correction` | `control.vb` (mean strategy) | `:none` | **diverges loudly**: modern R-INLA enables the low-rank VB mean correction (van Niekerk & Rue 2024) by default in compact mode; we ship it opt-in (`:mean` / `VBMeanCorrection(...)`) per ADR-048. The `control.vb` *variance* strategy is unpublished and not implemented. Flipping our default is a future decision gated on oracle re-runs with the correction active. |
